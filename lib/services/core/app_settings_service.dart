...
class AppSettingsNotifier extends ChangeNotifier {
  ...
  Future<void> downloadOnlineFont(String fontId, {FontDomain? domain}) async {
    ...
    if (isOnlineFontDownloaded(fontId)) {
      ...
      await _onlineFontService.ensureLoaded(
        fontId,
        files: _onlineFontFilesFromOption(option),
  ...
  /// 把 FontOption.downloadFiles（URL 字符串列表）转换为 OnlineFontFile 列表。
  /// 文件名取 URL 末段，未知大小用 0 占位（ensureLoaded/download 仅在校验时使用）。
  List<OnlineFontFile> _onlineFontFilesFromOption(FontOption option) {
    return option.downloadFiles
        .map((url) {
          final segment = url.split('/').last;
          final fileName = segment.isEmpty ? '${option.id}_${url.hashCode}' : segment;
          return OnlineFontFile(url: url, fileName: fileName, size: 0);
        })
        .toList(growable: false);
  }
  ...
  Future<void> _restoreSelectedFonts(SharedPreferences prefs) async {
    ...
    if (appOption.isCustom) {
      ...
      if (isOnlineFontDownloaded(_appFontId)) {
        await _onlineFontService.ensureLoaded(
          _appFontId,
          files: _onlineFontFilesFromOption(appOption),
    ...
    if (readerOption.isCustom) {
      ...
      if (isOnlineFontDownloaded(_readerFontId)) {
        await _onlineFontService.ensureLoaded(
          _readerFontId,
          files: _onlineFontFilesFromOption(readerOption),
